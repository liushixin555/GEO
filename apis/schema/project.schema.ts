...
export const createProjectSchema = z.object({
  ...
  company_id: z.number({ error: '公司ID不能为空' }).int().positive('公司ID必须为正整数'),
  operator_ids: z.array(z.number().int().positive()).min(1, '运营者不能为空').max(100),
...
export const updateProjectSchema = z.object({
  ...
  description: z.string().max(2000, '项目描述不能超过2000个字符').trim().optional(),
  operator_ids: z.array(z.number().int().positive()).min(1, '运营者不能为空').max(100).optional(),
  viewer_ids: z.array(z.number().int().positive()).max(100).optional(),
