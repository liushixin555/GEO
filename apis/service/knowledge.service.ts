...
export interface IKeywordService {
  ...
  batchCreate(projectId: number, keywords: string[], userId: number, groupId: number): Promise<KnowledgeKeyword[]>;
  listByGroup(groupId: number): Promise<KnowledgeKeyword[]>;
  syncGroup(groupId: number, projectId: number, keywords: string[], userId: number): Promise<KnowledgeKeyword[]>;
