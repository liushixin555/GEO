...
// SEC-APP-05: Trust proxy count from config — adapts to deployment architecture
app.set('trust proxy', config.server.trustProxy);

// SEC-APP-02: Health check with lightweight rate limit (before security middleware)
const healthLimiter = rateLimit({
  windowMs: 60_000,
  max: 1000,
  standardHeaders: true,
  legacyHeaders: false,
  message: { code: 429, message: '健康检查请求过于频繁' },
});
...
// SEC-APP-04: Request body parsing — 1MB limit for general API (upload routes use multer)
...
// Swagger API documentation — spec generated by swagger-autogen-ast
// Regenerate: npx ts-node apis/scripts/generate-swagger.ts
if (config.swagger.enabled) {
  const swaggerUI = require('swagger-ui-express');
  const swaggerSpec = require('./swagger-spec.json');
  // SEC-APP-06: CSP for Swagger UI — restrict inline scripts/styles to self only
