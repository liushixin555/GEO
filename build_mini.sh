...
setup_database() {
  ...
  # 使用 Docker 容器
  if docker ps --filter "name=by-db" --filter "status=running" | grep -q "by-db"; then
    echo "Docker 数据库容器已在运行"
  elif docker ps -a --filter "name=by-db" | grep -q "by-db"; then
    echo "Docker 数据库容器已存在但未运行，正在启动..."
    docker start by-db
  else
    echo "正在创建 Docker 数据库容器..."
    docker run -d --name by-db --restart=always -p 5432:5432 \
  ...
  for i in $(seq 1 15); do
    if docker exec by-db pg_isready -U postgres -d geo_ts -q 2>/dev/null; then
      echo "数据库已就绪"
      return 0
    fi
    sleep 1
...
# docker build
echo "正在构建 Docker 镜像..."
docker build -t by:latest .

# 停掉并删除之前的应用容器
docker stop by 2>/dev/null || true
docker rm by 2>/dev/null || true
...
echo "正在启动应用容器..."
docker run -d --name by --restart=always -p 12380:80 \
  --env-file "$DOCKER_ENV" \
