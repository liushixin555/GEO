...
describe('System Config Controller', () => {
  ...
  describe('PUT /api/system-configs', () => {
    it('should return 400 when configs is empty', async () => {
      const response = await agent
    ...
    it('should batch update configs successfully', async () => {
      const { getPrisma } = require('../../apis/utils/db.util');
      const mockUpsert1 = jest.fn().mockResolvedValue({
        id: 1, configKey: 'yishangshu_username', configValue: 'new_user', createdAt: new Date(), updatedAt: new Date(),
      });
      const mockUpsert2 = jest.fn().mockResolvedValue({
        id: 2, configKey: 'yishangshu_password', configValue: 'new_pass', createdAt: new Date(), updatedAt: new Date(),
      });
      const mockTransaction = jest.fn().mockResolvedValue([
        mockUpsert1(), mockUpsert2(),
      ]);
      getPrisma.mockReturnValue({ systemConfig: { upsert: mockUpsert1 }, $transaction: mockTransaction });
