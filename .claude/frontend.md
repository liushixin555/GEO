...
- **MarkdownEditor 封装组件**：`pages/components/MarkdownEditor.tsx`，封装 `@uiw/react-md-editor/nohighlight`
  ...
  - **commandsFilter 覆盖机制**：拦截第三方库命令并覆盖
    ...
    - **list.tsx 安全评审**（APPROVE，综合安全评分 7.5/10，攻击面极小）：
      ...
      - MEDIUM-03: `insertBeforeEachLine` 函数分支多重求值（每行最多 5 次调用），未复用 `insertionResult` 变量
      ...
      - **list.tsx 评审问题已修复**（2026-05-25，node_modules 内非持久化）：
        - P1: checkedList 回调增加 startsWith 检测，修复 toggle 已勾选项
        - P1: prefix! 改为防御性检查
        - P2: Remove 分支统一使用 state1、快捷键改为 ctrlcmd、Array.join 改为 repeat、快捷键重映射避免 DevTools 冲突
        - P3: unorderedList SVG 补充 role="img" + 三个 SVG 添加 aria-hidden="true"
        - 注意：封装层仍需覆盖 aria-label/title 为中文
...
- **文章详情/编辑页** `/article/:id`：独立路由页面（非Modal）
  ...
  - 文章设置 Tab：表单字段（关键词/画像/图片均为知识库纯选择），非编辑状态时 disabled
  - **两个操作按钮（仅 draft 状态显示）**：「提交给AI」（AI模式）/「提交」（手工模式）
