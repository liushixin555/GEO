...
**评审角色**: 软件架构专家
**评审日期**: 2026-05-25
**评审结论**: ✅ APPROVE（通过）—— 依赖拓扑单向无环、废弃策略规范、ICommand 契约完整，仅存在非空断言和图标风格两项中等风险
...
│    heading4: ICommand        四级标题命令定义              │
│    title4: ICommand          废弃别名                     │
├─────────────────────────────────────────────────────────┤
│  commands/title1-3,5-6.tsx   同构的兄弟文件                │
│    heading1-3,5-6: ICommand  其他级别标题命令              │
└─────────────────────────────────────────────────────────┘
```

该文件在命令系统中是**叶节点命令定义文件**，职责单一：定义四级标题的命令对象（`heading4`）并提供废弃别名（`title4`）。
...
| 命名一致性 | 7 | heading4/title4 双重导出增加认知成本，但废弃策略规范 |
...
title4.tsx（本文件）
     ...
     │ import ICommand etc. ────────┘
     │
     │ export heading4, title4 ───→ index.ts（注册到命令列表）
```

### 与 title1.tsx 的架构演进对比

title1.tsx 早期版本存在循环依赖（`title.tsx ↔ title1.tsx`）。**当前版本的 title4.tsx 已完全消除此问题**：
...
当前 title4.tsx（无循环依赖）:
  title4.tsx ──import headingExecute──→ headingUtils.ts（独立模块，无反向引用）
```

`headingExecute` 已从 `title.tsx` 提取为独立的 `headingUtils.ts`，title4.tsx 直接导入工具函数，**完全脱离循环依赖链**。这是库维护者在 v4.0.0 版本中做出的关键架构决策——将共享逻辑下沉到独立的叶子模块。
...
| 可替换性 | ✅ 高 | 可独立替换 heading4 的 execute 实现而不影响其他命令 |
...
|---|---|---|---|
| `name` | `'heading4'` | ✅ | 唯一标识，与 keyCommand 一致 |
| `keyCommand` | `'heading4'` | ✅ | 命令路由键，用于 CommandOrchestrator 分发 |
| `shortcuts` | `'ctrlcmd+4'` | ✅ | 跨平台自动映射（macOS: Cmd+4, Win/Linux: Ctrl+4） |
| `prefix` | `'#### '` | ✅ | 标准 Markdown H4 语法 |
| `suffix` | `''` | ✅ | 标题无后缀 |
...
| **heading4（本文件）** | **`<div>` 纯文本 + 内联样式** | **光栅化** | **❌** |

**影响范围**:
- 高 DPI / 缩放场景下可能模糊
- `fontSize: 14` 硬编码，无法被主题系统覆盖
- `"Heading 4"` 文本硬编码为英文，i18n 不友好
...
等差预期为 `18, 16, 14, 12, 10, 8`（每级 -2），但实际递减不规则（-2, -1, -1, -2, -1）。可能是有意为之（避免中间/低级别字号过小影响可读性），也可能是数值疏忽。H4 的 14px 作为工具栏图标可接受。
...
| 检查项 | 状态 | 说明 |
|---|---|---|
| 别名指向正确 | ✅ | `title4 = heading4` 引用赋值，运行时行为一致 |
| @deprecated 标记 | ✅ | JSDoc 标记存在，IDE 可显示删除线 |
| 废弃版本号 | ✅ | 标注 `Since v4.0.0` |
| 计划移除版本 | ✅ | 明确 `v5.0.0` |
| 迁移路径 | ✅ | `@see heading4` 指向替代方案 |
| 注释无矛盾 | ✅ | 语义清晰，无 title1.tsx 早期版本的自相矛盾问题 |
| 实际使用一致 | ❌ | `getCommands()` 仍使用 `title4` 而非 `heading4`（桶文件层面） |

**与 title1.tsx 早期版本的对比**: title1.tsx 的废弃注释曾自相矛盾（"Use `heading1`" 又说 "Use `title1`"），title4.tsx 已修正此问题，注释模板规范化。说明库维护者在后续文件中改进了废弃注释质量。
...
export const heading3 = createHeadingCommand(3);
export const heading4 = createHeadingCommand(4);
export const heading5 = createHeadingCommand(5);
export const heading6 = createHeadingCommand(6);
```
...
## 九、与 title1.tsx / title3.tsx 架构评审的对比

| 维度 | title1.tsx（早期评审） | title3.tsx（架构评审） | title4.tsx（本次评审） |
|---|---|---|---|
| 循环依赖 | 🔴 参与 title.tsx 循环 | ✅ 无循环依赖 | ✅ 无循环依赖 |
| 非空断言 | 🔴 prefix! 风险 | 🔴 prefix! 仍然存在 | 🔴 prefix! 仍然存在 |
| 废弃注释 | 🔴 自相矛盾 | ✅ 规范（含版本号+@see） | ✅ 规范（含版本号+@see） |
| 依赖方向 | ⚠️ 双向（被动参与） | ✅ 单向（叶节点→工具模块） | ✅ 单向（叶节点→工具模块） |
| 图标风格 | ⚠️ 内联样式 | ⚠️ 内联样式（同问题） | ⚠️ 内联样式（同问题） |
| 代码重复 | ⚠️ 6 文件同构 | ⚠️ 6 文件同构 | ⚠️ 6 文件同构 |
| 综合评分 | 6.1/10 | 7.7/10 | **7.7/10** |

**关键架构改善**: title4.tsx 与 title3.tsx 处于同一架构质量水平，均从循环依赖进化为单向依赖，废弃注释规范化。title1.tsx 的 6.1 分主要因循环依赖和注释矛盾被扣分。
...
| 🟡 中 | 移除废弃导出 | 删除 title4 别名 | 公共 API 缩减 |
...
| 升级兼容性 | 🟡 中 | 若项目通过 `import { title4 }` 引入，v5.0.0 移除后需迁移为 `heading4` |
...
2. **废弃策略规范** — `@deprecated` 含版本号（v4.0.0）、移除计划（v5.0.0）、`@see` 引用，文档质量优于 title1.tsx 早期版本
3. **职责单一** — 文件仅定义 heading4 命令和废弃别名，不承担额外职责
...
1. **非空断言** — `prefix!` 绕过类型保护，与 headingExecute 的参数契约不匹配（系统性问题，非本文件独有）
...
本文件架构质量良好（7.7/10），与 title3.tsx 处于同一水平，相比 title1.tsx 早期版本有显著改善——循环依赖已完全消除、废弃注释规范化、依赖方向正确。**唯一的实质性架构缺陷**是 `prefix!` 非空断言（系统性问题，非本文件独有）。**最有效的改善路径**是将 6 个同构的 titleN.tsx 合并为工厂函数 `createHeadingCommand(level)`，一次性解决代码重复、非空断言和 fontSize 不一致三个问题。对 by_geo 项目当前无功能性影响。
