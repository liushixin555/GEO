...
|---|---------|------|---------|------|---------|
| 1 | MEDIUM | 类型安全 | `state.command.prefix!` 非空断言绕过编译期 null 检查，若对象被篡改可导致 undefined 传入执行函数 | L14 | 改用 `state.command.prefix ?? '### '` 防御性默认值 |
| 2 | LOW | 一致性 | `suffix` 未与 `prefix` 使用相同的防御性处理方式（虽然下游有默认值兜底） | L14 | 统一使用 `??` 运算符 |
