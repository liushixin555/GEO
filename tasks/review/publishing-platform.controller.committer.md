...
| 优先级 | 问题 | 修复方案 | 预估工时 |
|--------|------|----------|---------|
| P2 | list 函数 err.message 可能泄露内部信息 | 500 错误统一返回通用消息 | 0.5h |
| P2 | 缺少 Swagger 文档注解 | 补全 sync + list 的 API 文档 | 1h |
| P2 | sync 并发无控制 | Service 层添加互斥锁或 debounce | 2h |

### 8.3 技术债务（中长期规划）

| 优先级 | 问题 | 修复方案 |
|--------|------|----------|
| P3 | 分页参数解析项目级重复 | 提取通用 `parsePagination()` 工具函数 |
| P3 | 错误分类基于字符串匹配 | 引入 `BusinessError` 自定义异常 |
| P3 | 同步机制硬编码单一数据源 | 引入数据源策略接口 |
