...
- **修复**: ✅ 已修复 — 移除了 title.tsx 对 heading1 的导入，heading 显式定义所有属性（含独立 execute）
...
- **修复**: ✅ 已修复 — 全部 title1-6 统一使用 `state.command.prefix ?? '# '` 等防御性默认值

### P2-01 [MEDIUM] 废弃注释自相矛盾 — title1-6
- **描述**: 注释说 "Use `heading1` instead" 但下一行又说 "Use `title1` for inserting Heading 1"——指向已被废弃的别名
- **影响**: API 消费者混淆
- **修复**: ✅ 已修复 — 统一为 `@deprecated Since v4.0.0. Use headingN instead. Scheduled for removal in v5.0.0. @see headingN`
...
- **修复**: ✅ 已修复 — 全部 title.tsx/title1-6 改为 `import type`
...
- **影响**: 无障碍缺失、视觉风格不一致
- **修复**: ✅ 已修复 — 全部 title1-6 添加 `role="img" aria-hidden="true"`、IBM Plex Sans 字体、Carbon ink 颜色 `#161616`、简写文本（H1-H6）

### P2-04 [LOW] heading 命令通过 spread 继承 heading1
- **描述**: `heading = { ...heading1, icon: ... }` 隐式继承所有属性，不利于维护
- **影响**: heading1 属性变更会意外影响 heading
- **修复**: ✅ 已修复 — heading 显式定义所有属性（name, keyCommand, shortcuts, prefix, suffix, buttonProps, icon, execute）
...
- **修复**: ✅ 已修复 — headingExecute 的 suffix 默认值从 `= prefix` 改为 `= ''`

### P3-02 [LOW] 废弃注释缺少版本信息
- **描述**: title.tsx 的废弃注释缺少具体的版本号（Since v4.0.0, Will be removed in v5.0.0）
- **修复**: ✅ 已修复 — title.tsx 废弃注释补充版本信息 `Since v4.0.0. Scheduled for removal in v5.0.0.`
