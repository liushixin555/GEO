...
### 5. 边界情况 (6个) ✨ 更新
- windowMs 为最小值 1 时应正确传递 ✨ 新增
- windowMs 为 0 时应抛出配置错误 ✨ 新增（修复：config 验证 min=1）
- max 为最小值 1 时应正确传递 ✨ 新增
- max 为 0 时应抛出配置错误 ✨ 新增（修复：config 验证 min=1）
- 极大的 max 值应正确传递
...
## 本次修复
- 原 `windowMs 为 0 时应正确传递` 和 `max 为 0 时应正确传递` 两个测试失败
- 原因：config 的 `safeParseInt` 函数对 rateLimit 参数设置了 `min: 1`，值 ≤0 会抛出错误
- 修复：拆分为合法最小值测试（1）和非法值错误测试（0）
- 测试数：19 → 21
