...
此前 87.5% 的函数覆盖率是因为 Swagger JSON 处理函数（line 102）仅在 `SWAGGER_ENABLED=true` 时注册，isolated 测试创建了独立 app 实例但覆盖率工具未计入。本次通过在 isolated 测试中同时 mock swagger-auth 中间件，使该函数被正确执行并计入覆盖率。
